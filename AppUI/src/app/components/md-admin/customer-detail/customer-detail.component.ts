...
export class CustomerDetailComponent implements OnInit {
  ...
  onUpdateCustomer() {
    this.isSubmitting = true;
    this.editing_Customer.modified_date = formatDate(this.customerDateStart, 'yyyy-MM-dd', 'en-US', '+0530');
    if (this.genderMale)
      this.editing_Customer.gender = 'Male'
    else if (this.genderFemale)
      this.editing_Customer.gender = 'Female'
    this.apiService.update(`${this.apiService.apiUrl}/sell-goods/customers/${this.editing_Customer.id}`, this.editing_Customer).subscribe((data) => {
      this.onSuccess.emit();
      Utils.notifySuccess('Customer has been added successfully.');
      this.isSubmitting = false;
    }, (error) => {
